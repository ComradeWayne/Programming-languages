...
public:
	Vector(int a, float* m) // обычный конструктор
	{
		n = a;
		num = new float[n];
		for (int i = 0; i < n; i++)
	...
	float mod()//вычисление модуля вектора
	{
		float a = 0;
		for (int i = 0; i < n; i++)
			a += num[i] * num[i];
	...
	float scalar(Vector V2)//вычисление скалярного произведения векторов
	{
		float a = 0;
		if (n > V2.get_dim())
		{
